...
class TuiApp(App):
    ...
    def __init__(self, config: Config, **kwargs) -> None:
        ...
        self.progress_bars: list[RichProgressBar] = []
    ...
    def update_table(self, value_index: int) -> None:
        ...
        table.update_cell_at((value_index, 0), val_stat["val"])
        table.update_cell_at((value_index, 1), progress_bar)
        table.update_cell_at((value_index, 2), reps_done)
        table.update_cell_at((value_index, 3), mean_str)
        table.update_cell_at((value_index, 4), median_str)
        table.update_cell_at((value_index, 5), min_str)
        table.update_cell_at((value_index, 6), max_str)


def tui_main(config: Config) -> DefaultDict[str, list]:
    """
